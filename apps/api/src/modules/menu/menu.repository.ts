...
import { MenuEntity, MenuItemEntity } from '../../database/entities';
import { RequestContextService } from '../../common/services/request-context.service';
import { TenantScopedRepository } from '../../database/repositories/tenant-scoped.repository';

@Injectable()
export class MenuRepository extends TenantScopedRepository<MenuEntity> {
  constructor(
    @InjectEntityManager() manager: EntityManager,
    context: RequestContextService,
  ) {
    super(manager, MenuEntity, context);
  }

  async findByCode(storeId: string, code: string): Promise<MenuEntity | null> {
    return this.findOne({ where: { storeId, code } });
  }
...
@Injectable()
export class MenuItemRepository extends TenantScopedRepository<MenuItemEntity> {
  constructor(
    @InjectEntityManager() manager: EntityManager,
    context: RequestContextService,
  ) {
    super(manager, MenuItemEntity, context);
  }

  async findByMenu(menuId: string): Promise<MenuItemEntity[]> {
    return this.find({ where: { menuId }, order: { sortOrder: 'ASC' } });
  }
}

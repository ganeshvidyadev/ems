...
**Status: Phase 1 (Discovery) complete. Phase 2 (Smoke) complete. Phase 6 (Security) complete — P0 found and fixed.**
...
| KF-07 | P2 | Contract correctness (open) | `ProductResponse.storeId`/`brandId`/`taxClassId` and `OrderItemResponse.productId`/`variantId` return internal ids, not public ids, the same class of bug as KF-04 | **Open** |
...
| 5 | API testing | ✅ Executable, 264 routes is large but tractable in batches | Prioritize by business criticality, not alphabetical |
...
| 7 | Business workflows | ✅ Executable end-to-end (Product→Inventory→Cart→Checkout→Order verified once already this session) | Repeat systematically per workflow |
